...
const themeStyles =
  ".themes-container{display:flex;align-items:center;justify-content:center;flex-direction:column;width:500px}.themes-container ul{border:2px solid #fff;padding:0;display:flex;flex-direction:column;width:100%}.themes-container ul li{list-style-type:none;display:flex;flex-direction:row}.themes-container ul li div{background-color:orange;height:50px;width:25%}.themes-container ul li:hover{cursor:pointer;filter:brightness(80%)}@media (max-width: 1000px){.themes-container{width:60vw}}";
...
async function fetchDependencies() {
  let response = await fetch(
    "https://raw.githubusercontent.com/SintcoLTD/sparxmaths/main/release.json"
  );
...
if (colours == null) {
  ...
  document.documentElement.style.setProperty(
    "--darkest",
    themes[colours.theme]["darkest"]
  );
  document.documentElement.style.setProperty(
    "--dark",
    themes[colours.theme]["dark"]
  );
  document.documentElement.style.setProperty(
    "--light",
    themes[colours.theme]["light"]
  );
  document.documentElement.style.setProperty(
    "--lightest",
    themes[colours.theme]["lightest"]
  );
}
...
const apiKey =
  "ZTxLUr2NMrbV12Bn5CGA5qGfmoG48pBhQAyEnPsClNqmVsPmAe4OM4R8C2I5T46z";
...
async function checkUser() {
  ...
  if (!(hash in users)) {
    ...
    let newDocumentId = await (
      await contactDatabase("insertOne", "answers", "user-data", {
        answers: {},
      })
    ).insertedId;
  ...
  let userDocument = await (
    await contactDatabase("findOne", "answers", "user-data", {
      _id: { $oid: userDocumentId },
    })
  ).document;
...
async function authorise() {
  ...
  if (timeAlive >= 1740) {
    ...
    let response = await (
      await fetch(
        `https://realm.mongodb.com/api/client/v2.0/app/${appName}/auth/providers/api-key/login`,
        settings
      )
    ).json();
...
async function main() {
  ...
  if (
    document.querySelector(".status-bar-label.status-bar-label-username") !==
      null &&
    userName == null
  ) {
    userName = document.querySelector(
      ".status-bar-label.status-bar-label-username"
    ).textContent;
  }
  ...
  let correctStatusElement = document.querySelector(
    ".page.result .result-inner .correct"
  );
  if (
    correctStatusElement !== null &&
    document.querySelector("#shown-answer") === null
  ) {
    try {
  ...
  const bookworkCodeElement = document.querySelector(
    ".wac-text-container .bookwork-code"
  );
  if (
    bookworkCodeElement !== null &&
    document.querySelector("#custom-answer") === null
  ) {
    try {
      let bookworkCode = bookworkCodeElement.textContent;
      bookworkCode = bookworkCode.replace("Bookwork code: ", "");
      ...
      bookworkAnswer = bookworkAnswer.map((element) =>
        element.replace("\n", "")
      );
      bookworkAnswer = bookworkAnswer.map((element) =>
        element.replace(/\\\\/g, "\\")
      ...
      if (answerOptions !== null) {
        for (let i = 0; i < answerOptions.length; i++) {
          ...
          answerOptions[i].querySelector(
            ".answer-markup.choice-wac-option"
          ).style.border = "5px solid var(--orange)";
  ...
  if (questionContainer !== null) {
    ...
    questionImages = document.querySelectorAll(
      '[data-test-target="image-img"]'
    );
...
document.addEventListener("click", function (e) {
  if (e.target) {
    try {
      if (
        (e.target.id == "skill-delivery-submit-button" &&
          e.target.innerText == "Submit") ||
        (e.target.className == "button-text" &&
          e.target.textContent == "Submit") ||
        (e.target.className == "button-all" &&
          e.target.textContent == "Submit") ||
        (e.target.className == "button-icon button-icon-right" &&
          e.target.parentElement.innerText == "Submit") ||
        (e.target.parentElement.className == "button-icon button-icon-right" &&
          e.target.parentElement.parentElement.innerText == "Submit") ||
...
document.addEventListener("keypress", function (event) {
  if (event.key === "Enter") {
    ...
    const revisionItems = document.querySelectorAll(
      ".revision-task-item.btn-menu-item"
    );
    ...
    if (minigames.length > 0) {
      minigames.forEach(function (minigame) {
        if (
          minigame.querySelector(".minigame-title").textContent ==
...
function hasSource(answer) {
  if (
    answer.toString().includes("https") ||
    answer.toString().includes("http")
  ) {
    return true;
  } else {
    return false;
...
function getInput() {
  ...
  const chosen = document.querySelectorAll(
    ".answer-part .gap-card.selected, .choice.selected"
  );
  addText(chosen);
...
async function credits() {
  ...
  console.log.apply(console, [
    "%c Thanks for using my Sparx program! ",
    "color: #fff; background: #8000ff; padding:5px 0;",
  ]);
  console.log.apply(console, [
    "%c Designed and Developed by Alex lo Storto %c\ud83d\ude80 ",
    "color: #fff; background: #8000ff; padding:5px 0;",
